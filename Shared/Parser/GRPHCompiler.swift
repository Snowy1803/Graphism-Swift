...
class GRPHCompiler {
    ...
    func compile() -> Bool {
        ...
        for lineNumber in 0..<lines.count {
            ...
            do {
                let resolved = try resolveInstruction(literal: tline)
                if let resolved = resolved {
                    if resolved.ignoreBlocks {
                        try addNonBlockInstruction(resolved.instruction)
                    } else {
    ...
    /// Resolves an instruction. Used both for lines and void lambdas.
    ///
    /// - Parameter tline: the literal
    /// - Returns: an instruction if one was found, nil if the line should be ignored or has been handled
    func resolveInstruction(literal tline: String) throws -> (instruction: Instruction, ignoreBlocks: Bool)? {
        if tline.hasPrefix("#") {
            if tline.hasPrefix("#!") {
                return nil // shebang, ignore
            }
            // MARK: COMMANDS
            ...
            case "#else":
                ...
                return (ElseBlock(context: &context, lineNumber: lineNumber), false)
            case "#while":
                return (try WhileBlock(lineNumber: lineNumber, context: &context, condition: Expressions.parse(context: context, infer: SimpleType.boolean, literal: params)), false)
            case "#foreach":
                let split = params.split(separator: ":", maxSplits: 1)
                guard split.count == 2 else {
                    throw GRPHCompileError(type: .parse, message: "'#foreach varName : array' syntax expected; array missing")
                }
                return (try ForEachBlock(lineNumber: lineNumber, context: &context, varName: split[0].trimmingCharacters(in: .whitespaces), array: Expressions.parse(context: context, infer: ArrayType(content: SimpleType.mixed), literal: split[1].trimmingCharacters(in: .whitespaces))), false)
            case "#try":
                return (TryBlock(context: &context, lineNumber: lineNumber), false)
            case "#catch":
                let split = params.split(separator: ":", maxSplits: 1)
                guard split.count == 2 else {
            ...
            case "#throw":
                ...
                return (ThrowInstruction(lineNumber: lineNumber, type: error, message: exp), false)
            case "#function":
            ...
            case "#return":
                ...
                return (ReturnInstruction(lineNumber: lineNumber, value: aexp), false)
            case "#break":
                return (try BreakInstruction(lineNumber: lineNumber, type: .break, scope: .parse(params: params)), false)
            case "#continue":
                return (try BreakInstruction(lineNumber: lineNumber, type: .continue, scope: .parse(params: params)), false)
            case "#fall":
                return (try BreakInstruction(lineNumber: lineNumber, type: .fall, scope: .parse(params: params)), false)
            case "#fallthrough":
                return (try BreakInstruction(lineNumber: lineNumber, type: .fallthrough, scope: .parse(params: params)), false)
            case "#goto":
                throw GRPHCompileError(type: .unsupported, message: "#goto has been removed")
            case "#block":
                return (SimpleBlockInstruction(context: &context, lineNumber: lineNumber), false)
            case "#requires":
            ...
            // arr{4} = var
            return (try ArrayModificationInstruction(lineNumber: lineNumber, context: context, groups: result), false)
            ...
            var inner = context!
            return (try FunctionDeclarationBlock(lineNumber: lineNumber, context: &inner, def: tline), true)
        } else if let result = VariableDeclarationInstruction.pattern.firstMatch(string: tline) {
            // {integer} arr = (0 1 2 3)
            return (try VariableDeclarationInstruction(lineNumber: lineNumber, groups: result, context: context), false)
        } else if let result = AssignmentInstruction.pattern.firstMatch(string: tline) {
            // assignments (=, +=, /= etc)
            return (try AssignmentInstruction(lineNumber: lineNumber, context: context, groups: result), false)
            ...
            if let onLiteral = result[2] {
                ...
                return (ExpressionInstruction(lineNumber: lineNumber, expression: try MethodExpression(ctx: context, method: method, on: on, values: try Expressions.splitParameters(context: context, in: result[3]!, delimiter: Expressions.space), asInstruction: true)), false)
            } else {
                // function or method on this
                if let function = Function(imports: imports, namespace: ns, name: member.member) {
                    return (ExpressionInstruction(lineNumber: lineNumber, expression: try FunctionExpression(ctx: context, function: function, values: try Expressions.splitParameters(context: context, in: result[3]!, delimiter: Expressions.space), asInstruction: true)), false)
                } else if let method = Method(imports: imports, namespace: ns, name: member.member, inType: context.findVariable(named: "this")!.type) {
                    return (ExpressionInstruction(lineNumber: lineNumber, expression: try MethodExpression(ctx: context, method: method, on: VariableExpression(name: "this"), values: try Expressions.splitParameters(context: context, in: result[3]!, delimiter: Expressions.space), asInstruction: true)), false)
                } else {
            ...
            return (try ExpressionInstruction(lineNumber: lineNumber, expression: FuncRefCallExpression(ctx: context, varName: result[1]!, values: Expressions.splitParameters(context: context, in: result[2]!, delimiter: Expressions.space), asInstruction: true)), false)
            ...
            return (try ExpressionInstruction(lineNumber: lineNumber, expression: ArrayValueExpression(context: context, varName: result[1]!, index: index.isEmpty ? nil : Expressions.parse(context: context, infer: SimpleType.integer, literal: index), removing: true)), false)
        } else {
